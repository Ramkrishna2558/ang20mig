...
- Do not use Signal Forms (`@angular/forms/signals`) in this Angular 20.3.17 project. Official Angular docs mark Signal Forms as experimental and requiring Angular v21 or higher. Use typed Reactive Forms for production form migrations unless the user explicitly upgrades Angular and accepts experimental APIs.
- Prefer Reactive Forms for complex enterprise forms, cross-field validation, async validation, dynamic controls, and testable validation logic.
- Template-driven forms may be used only for very small forms when that matches existing Angular project conventions.
...
Run the lightest relevant validation from `gaws/gaws02/client-ngx`:
...
When the migration framework expects tracking, create or update:
...
- Do not claim the migration is complete if legacy template, route, CSS, API, or validation evidence was missing.
